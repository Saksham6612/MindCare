  ...
  return (
    ...
    >
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-xs sm:text-sm font-extrabold uppercase tracking-wider text-purple-800 bg-purple-100 px-3 py-0.5 rounded-full">
              {t('home.medicine_reminder') || 'Medication'}
            </span>
            {isTaken && (
              <span className="text-xs sm:text-sm font-extrabold text-emerald-800 bg-emerald-100 px-2.5 py-0.5 rounded-full flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5" /> {t('reminders.status_taken', { defaultValue: 'Taken' })}
              </span>
            )}
          </div>
          ...
          <h3 className={`text-2xl sm:text-3xl font-extrabold tracking-tight ${
            ...
            {t('home.morning_tablet') || 'Morning Tablet'}
          </h3>

          <div className="flex items-center gap-1.5 text-base sm:text-lg font-bold text-purple-900 pt-0.5">
            <Clock className="w-5 h-5 text-purple-600" />
            <span>10:00 AM</span>
            <span className="text-gray-400 font-normal">• {t('home.tablet_with_water', { defaultValue: '1 Tablet with warm water' })}</span>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        aria-label={isTaken ? t('home.undo_taken', { defaultValue: 'Morning Tablet marked as taken. Tap to undo.' }) : t('home.mark_taken', { defaultValue: 'Mark as Taken' })}
      ...
      >
        {isTaken ? (
          <>
            ...
            <span>{t('reminders.status_taken', { defaultValue: 'Taken' })}</span>
          </>
        ) : (
          <>
            <CheckCircle2 className="w-6 h-6" />
            <span>{t('home.mark_taken') || 'Mark as Taken'}</span>
          </>
        )}
      </button>
    </section>
